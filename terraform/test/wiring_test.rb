...
class WiringTest < CisTestCase
  ...
  def test_every_stack_has_terraform_tfvars_example
    ALL_STACKS.each do |stack|
      assert File.file?(CisTest.stack_path(stack, "terraform.tfvars.example")),
             "#{stack}: no terraform.tfvars.example — operator cannot see required inputs"
    end
  end
  ...
  def test_each_stack_has_its_own_provider_tf
    ALL_STACKS.each do |stack|
      assert File.file?(CisTest.stack_path(stack, "provider.tf")),
             "#{stack}: no provider.tf — each stack is a self-contained Terraform root module"
    end
  end

  def test_the_provider_is_pinned
    src = Hcl.read(CisTest.stack_path("audit", "provider.tf"))
  ...
  def test_hcl_is_canonically_formatted
    ...
    dirs = MODULES.map { |m| CisTest.module_path(m) } +
