...
# The registry <-> HCL contract, applied to the Alibaba variant. Mirrors the
# checks WiringTest runs for tencent, against config/alibaba/controls.yml and
# stacks/aws/*. Azure has no shared modules, so the module checks are skipped.
class AlibabaWiringTest < CisTestCase
  include CisTest::Hcl

  AWS_STACKS = ([Cis::AUDIT_STACK] + Cis::HARDENING_STACKS.fetch("alibaba")).freeze

  def aws_path(stack, *parts)
    File.join(Cis::ROOT, "stacks", "alibaba", stack, *parts)
  end
  ...
  def aws_catalog
    @aws_catalog ||= Cis::Catalog.load(File.join(Cis::ROOT, "config", "alibaba", "controls.yml"))
  end
  ...
  def test_each_aws_stack_implements_exactly_what_the_registry_routes_to_it
    Cis::HARDENING_STACKS.fetch("alibaba").each do |stack|
  ...
  def test_the_aws_provider_is_pinned_in_every_stack
    AWS_STACKS.each do |stack|
      ...
      assert_match(%r{aliyun/alicloud}, src, "#{stack}: wrong provider")
      assert_match(/version\s*=\s*"[~>=\s]*\d+\.\d+/, src, "#{stack}: the provider version must be pinned")
    end
  end

  def test_alibaba_hcl_is_canonically_formatted
  ...
  def test_every_aws_control_id_mentioned_in_hcl_exists_in_the_registry
    ...
    Dir.glob(File.join(Cis::ROOT, "stacks", "alibaba", "**", "*.tf")).sort.each do |path|
      ...
      src.scan(/"(\d+\.\d+)"/).flatten.uniq.each do |id|
        assert_includes known, id,
                        ...
                        "which is not in config/alibaba/controls.yml"
      end
    end
  end
end
