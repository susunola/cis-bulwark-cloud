...
# reaches terraspace, terraform or a cloud API - what is being asserted is the
...
class CliTest < CisTestCase
  ...
  def test_only_filters_down_to_one_stack
    ...
    assert_equal 0, r.status, r
    assert_includes r.stdout, "terraspace up    storage"
    assert_includes r.stdout, "4.1, 4.3, 4.4, 4.5, 4.6, 4.7"
    refute_includes r.stdout, "up    network"
  ...
  def test_section_and_exclude_compose
    ...
    assert_equal 0, r.status, r
    assert_includes r.stdout, "terraspace up    network     # 3.1, 3.2, 3.4, 3.5, 3.6, 3.7"
    assert_includes r.stdout, "terraspace up    storage     # 4.1, 4.3, 4.4, 4.5, 4.7"
  ...
  def test_tag_can_span_stacks
    ...
    assert_equal 0, r.status, r
    assert_includes r.stdout, "terraspace up    logging"
    assert_includes r.stdout, "terraspace up    storage"
  end
  ...
  def test_plan_and_apply_differ_only_in_the_verb
    ...
    assert_includes plan.stdout,  "terraspace plan  storage"
    assert_includes apply.stdout, "terraspace up    storage"
  ...
  def test_apply_runs_stacks_in_the_declared_order
    r = cis("apply", "--dry-run")
    assert_equal 0, r.status, r
    order = r.stdout.scan(/terraspace up\s+(\w+)/).flatten
  ...
  def test_apply_never_schedules_a_stack_for_a_detect_only_control
    ...
    refute_includes r.stdout, "terraspace up"
  end
  ...
  def test_scan_targets_the_read_only_audit_stack
    ...
    assert_includes r.stdout, "terraspace up    audit"
    assert_includes r.stdout, "4.1, 4.2, 4.8, 4.9"
    Cis::HARDENING_STACKS.each { |s| refute_includes r.stdout, "up    #{s}" }
  end
  ...
  def test_scan_of_an_unobservable_selection_says_so_instead_of_reporting_pass
    ...
    refute_includes r.stdout, "terraspace"
  end
