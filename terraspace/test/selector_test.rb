...
class SelectorTest < CisTestCase
  ...
  def test_active_stacks_depends_on_the_action
    with_env("CIS_ONLY" => "4.*", "TS_CIS_ACTION" => "apply") do
      assert_equal %w[storage], Cis.active_stacks
    end
    with_env("CIS_ONLY" => "4.*", "TS_CIS_ACTION" => "scan") do
      assert_equal %w[audit], Cis.active_stacks
    end
    with_env("CIS_ONLY" => "4.*") do
      # No declared action: stay introspectable rather than hiding stacks.
      assert_equal ["audit"] + Cis::HARDENING_STACKS, Cis.active_stacks
    end
  end

  def test_scan_of_a_purely_manual_selection_activates_no_stack
    with_env("CIS_SECTIONS" => "9", "TS_CIS_ACTION" => "scan") do
      assert_empty Cis.active_stacks
    end
    with_env("CIS_SECTIONS" => "9", "TS_CIS_ACTION" => "apply") do
      assert_empty Cis.active_stacks
    end
  end
